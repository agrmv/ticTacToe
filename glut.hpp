...
/*class Board {
public:
    virtual void draw(const size_t& , const size_t& ) = 0;
    virtual ~Board() = default;
    static Board* create(const uint8_t &id);
};

class X: public Board {
public:
    void draw(const size_t& i, const size_t& j) {
        glBegin(GL_LINES);
...
class O: public Board {
public:
    void draw(const size_t& i, const size_t& j) {
        double x = j * scale + (scale / 2);
        double y = i * scale + (scale / 2);
        double r = scale / 2;
        uint32_t amountSegments = 36;

        glBegin(GL_LINE_LOOP);

        for (size_t i = 0; i < amountSegments; ++i) {
            double angle = 2. * M_PI * static_cast<double>(i) / static_cast<double>(amountSegments);
...
Board* Board::create(const uint8_t &id ) {
    Board* ptr;
    if (id) {
        ptr = new O();
    } else {
...
void drawCards() {
    std::vector<Board*> v;
    v.push_back(Board::create(0));
    v.push_back(Board::create(1));
    for (size_t i = 0; i < widthBoard; i++) {
        for (size_t j = 0; j < heightBoard; j++) {
            if (p.board[i][j] != -1) {
                glLineWidth(2);
                if (p.board[i][j] == 0) {
                    v[0]->draw(i, j);
